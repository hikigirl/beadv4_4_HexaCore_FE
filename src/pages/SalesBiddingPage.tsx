...
export const SalesBiddingPage = () => {
    ...
    useEffect(() => {
        if (sizeId) {
            ...
            getSellNowPrice(Number(sizeId)).then(res => {
                const price = res?.data?.sellNowPrice || null;
                setImmediatePrice(price);
                if (price) setMode('sell');
            });
            // 즉시 구매가 조회
            getBuyNowPrice(Number(sizeId)).then(res => {
                setImmediateBuyPrice(res?.data?.buyNowPrice || null);
            });
        }
    }, [sizeId]);
    ...
    const commissionFee = 0; // 현재는 0원
    const totalAmount = displayPrice - commissionFee; // 정산 예정 금액
        ...
        <div className="min-h-screen bg-[#FAFAFA] pt-[80px] pb-20 px-4 font-pretendard">
            <div className="max-w-[780px] mx-auto bg-white border border-gray-100 shadow-sm rounded-3xl overflow-hidden mt-8">
                ...
                <div className="p-8">
                    ...
                    >
                        {isSubmitting ? '처리 중...' : (mode === 'sell' ? '즉시 판매 계속' : '판매 입찰 등록')}
                    </button>
                </div>
            </div>
        </div>
